...
# if ls ./utils/ctc_token_fst 1> /dev/null 2>&1; then
# echo "Run the packaging program"
#./utils/ctc_token_fst $dir/tokens.txt | fstcompile --isymbols=$dir/tokens.txt --osymbols=$dir/tokens.txt \
# else
...
if ls ./utils/ctc_token_fst_corrected 1> /dev/null 2>&1; then
echo "Run the packaging program"
./utils/ctc_token_fst_corrected $dir/tokens.txt | fstcompile| fstarcsort --sort_type=olabel > $dir/T.fst || exit 1;
else
echo "Run the source file"
utils/ctc_token_fst_corrected.py $dir/tokens.txt | fstcompile| fstarcsort --sort_type=olabel > $dir/T.fst || exit 1;
fi
